...
- a `postgres` service container, health-gated on `--health-cmd` and reached
  at its service id;
- a tool supplied directly by the locked immutable runner profile, with no
  runtime package install or command shim;
...
offline against one
sandbox `$HOME`, because the cache and toolcache only prove anything if they
survive between runs, and Phase 6's verified content only proves offline
replay when the exact cached lock succeeds.
