...
mod tests {
    ...
    #[test]
    fn parses_a_copy_snapshot_with_fallback_context() {
        let snapshot = parse_status(
            "v=1 phase=copy strategy=copy-in fell_back=1 reason=EPERM files=1234 bytes=987654\n",
        )
        .expect("parses");
        assert_eq!(snapshot.phase, "copy");
        assert_eq!(snapshot.strategy.as_deref(), Some("copy-in"));
        assert!(snapshot.fell_back);
        assert_eq!(snapshot.reason.as_deref(), Some("EPERM"));
        assert_eq!((snapshot.files, snapshot.bytes), (1234, 987654));
        assert!(snapshot.detail.is_empty());
    }

    #[test]
    fn parses_failure_detail_lines() {
        let snapshot = parse_status("v=1 phase=failed\ncould not copy /x: denied\nsecond line\n")
            .expect("parses");
        assert_eq!(snapshot.phase, "failed");
        assert_eq!(snapshot.detail, "could not copy /x: denied\nsecond line");
    }

    #[test]
    fn rejects_unversioned_or_phaseless_text() {
        assert_eq!(parse_status("phase=copy files=1 bytes=2"), None);
        assert_eq!(parse_status("v=1 strategy=overlay"), None);
        assert_eq!(parse_status("garbage"), None);
        assert_eq!(parse_status(""), None);
    }

    #[test]
    fn stall_during_copy_names_counts_and_the_clean_clone_fix() {
        let snapshot =
            parse_status("v=1 phase=copy strategy=copy-in files=9000 bytes=123").expect("parses");
        let error = stall_failure(Some(&snapshot), &ReadinessConfig::default()).to_string();
        assert!(error.contains("9000 files"), "{error}");
        assert!(error.contains("clean clone"), "{error}");
        assert!(
            !error.contains("sh") || !error.contains("tail"),
            "the stall error never blames the image's shell: {error}"
        );
    }
}
