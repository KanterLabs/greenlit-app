...
mod tests {
    ...
    fn converts_scalars() {
        ...
        assert_eq!(to_json(&Value::Number(3.5)), serde_json::json!(3.5));
        assert_eq!(
            to_json(&Value::String("hi".to_string())),
            serde_json::json!("hi")
        );
    }
    ...
    fn converts_nested_arrays_and_objects() {
        let value = Value::object(vec![
            ("number".to_string(), Value::Number(1.0)),
            (
                "list".to_string(),
                Value::array(vec![Value::String("x".to_string()), Value::Bool(false)]),
            ),
        ]);
        let json = to_json(&value);
        assert_eq!(json["number"], serde_json::json!(1.0));
        assert_eq!(json["list"], serde_json::json!(["x", false]));
    }

    #[test]
    fn non_finite_numbers_become_null_rather_than_invented() {
