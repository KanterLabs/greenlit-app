...
mod tests {
    ...
    fn run(chunks: &[&[u8]], preset_masks: &[&str]) -> (String, usize, usize) {
        let mut out: Vec<u8> = Vec::new();
        let mut masker = Masker::new();
        for m in preset_masks {
            masker.add(m);
        }
        let (errors, warnings);
        {
            let mut sink = StepLogSink::new(&mut out, &mut masker);
            for chunk in chunks {
        ...
        (String::from_utf8(out).unwrap(), errors, warnings)
    }

    #[test]
    fn assembles_lines_across_chunk_boundaries() {
        let (out, _, _) = run(&[b"hel", b"lo\nwor", b"ld\n"], &[]);
        assert_eq!(out, "hello\nworld\n");
    }

    #[test]
    fn partial_final_line_is_flushed() {
        let (out, _, _) = run(&[b"no newline"], &[]);
        assert_eq!(out, "no newline\n");
    }

    #[test]
    fn add_mask_hides_the_value_and_redacts_later_lines() {
        let (out, _, _) = run(&[b"::add-mask::hunter2\n", b"pw=hunter2\n"], &[]);
        // The add-mask line never prints, and the later echo is redacted.
        assert_eq!(out, "pw=***\n");
    }

    #[test]
    fn preset_secret_is_masked_from_the_first_line() {
        let (out, _, _) = run(&[b"token=abc123\n"], &["abc123"]);
        assert_eq!(out, "token=***\n");
    }

    #[test]
    fn groups_indent_and_annotations_are_counted() {
        let (out, errors, warnings) = run(
            &[
                b"::group::Build\n",
                b"compiling\n",
                b"::endgroup::\n",
                b"::warning::careful\n",
                b"::error file=a.rs::boom\n",
            ],
            &[],
        );
        assert!(out.contains("\u{25be} Build\n"));
