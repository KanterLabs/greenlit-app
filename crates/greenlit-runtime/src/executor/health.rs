...
#[cfg(test)]
mod tests {
    use super::{apply, duration_nanos};
    use crate::engine::HealthCheck;

    #[test]
    fn go_durations_parse_the_way_docker_reads_them() {
        assert_eq!(duration_nanos("10s"), Some(10_000_000_000));
        assert_eq!(duration_nanos("500ms"), Some(500_000_000));
        assert_eq!(duration_nanos("1m30s"), Some(90_000_000_000));
        assert_eq!(duration_nanos("1h"), Some(3_600_000_000_000));
        assert_eq!(duration_nanos("2us"), Some(2_000));
        // A unitless value is seconds.
        assert_eq!(duration_nanos("5"), Some(5_000_000_000));
        // Not durations.
        assert_eq!(duration_nanos("soon"), None);
        assert_eq!(duration_nanos(""), None);
        assert_eq!(duration_nanos("10x"), None);
    }

    #[test]
    fn every_health_flag_is_interpreted() {
        ...
        for (flag, value) in [
            ...
            assert_eq!(
                apply(&mut health, flag, Some(value)),
                Ok(true),
                "{flag} must be interpreted, not rejected"
            );
        }
        assert_eq!(
            health.test,
            ...
            "--health-cmd runs through a shell, as it does on the command line"
        );
        assert_eq!(health.interval_nanos, Some(10_000_000_000));
        assert_eq!(health.timeout_nanos, Some(5_000_000_000));
        assert_eq!(health.start_period_nanos, Some(30_000_000_000));
        assert_eq!(health.retries, Some(5));
    }

    #[test]
    fn a_non_health_flag_is_left_to_the_existing_rules() {
        let mut health = HealthCheck::default();
        assert_eq!(apply(&mut health, "--privileged", None), Ok(false));
        assert_eq!(apply(&mut health, "--cpus", Some("1")), Ok(false));
    }

    #[test]
    fn an_unreadable_health_value_is_named_rather_than_ignored() {
        let mut health = HealthCheck::default();
        assert!(apply(&mut health, "--health-interval", Some("soon")).is_err());
        assert!(apply(&mut health, "--health-retries", Some("many")).is_err());
        assert!(apply(&mut health, "--health-cmd", None).is_err());
    }
}
