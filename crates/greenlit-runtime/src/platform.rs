...
mod tests {
    ...
    #[test]
    fn label_map_matches_supported_labels() {
        assert_eq!(
            resolve_base_image("ubuntu-latest"),
            Some(UbuntuRelease::Noble2404)
        );
        assert_eq!(
            resolve_base_image("ubuntu-24.04"),
            Some(UbuntuRelease::Noble2404)
        );
        assert_eq!(
        ...
        assert_eq!(resolve_base_image("macos-14"), None);
        assert_eq!(resolve_base_image("ubuntu-latest-arm"), None);
    }

    #[test]
    fn release_versions_and_refs() {
        assert_eq!(UbuntuRelease::Noble2404.version(), "24.04");
        assert_eq!(UbuntuRelease::Jammy2204.version(), "22.04");
        assert_eq!(UbuntuRelease::Noble2404.base_image_ref(), "ubuntu:24.04");
        assert_eq!(UbuntuRelease::Jammy2204.base_image_ref(), "ubuntu:22.04");
    }
}
