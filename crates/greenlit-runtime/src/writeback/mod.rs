...
mod tests {
    ...
    #[test]
    fn no_input_rejects_write_back_with_a_fix_message() {
        let err = validate_request(true, true).unwrap_err();
        let message = err.to_string();
        assert!(message.contains("--write-back"));
        assert!(message.contains("--no-input"));
        // The message states the resolving action (drop / omit a flag).
        assert!(message.contains("drop") || message.contains("omit"));

        // Every other combination is permitted.
        assert!(validate_request(true, false).is_ok());
        assert!(validate_request(false, true).is_ok());
        assert!(validate_request(false, false).is_ok());
    }

    #[test]
    fn interactive_confirm_accepts_yes_and_rejects_otherwise() {
        let changes = [Change {
        ...
        for (answer, expected) in [
            ...
            let mut out = Vec::new();
            let mut confirm = InteractiveConfirm::new(answer.as_bytes(), &mut out, "/repo");
            assert_eq!(confirm.confirm(&changes), expected, "answer {answer:?}");
            let rendered = String::from_utf8(out).expect("utf8");
            assert!(rendered.contains("a.txt"), "prompt lists the change");
        }
    }
}
