...
fn prepare_isolation(args: &Args, status: &StatusWriter) -> Result<(), InitError> {
    ...
    match args.strategy {
        StrategyPref::CopyIn => {
            let stats = populate_with_status(args, status, None)?;
            status.done("copy-in", None, Some(stats));
            Ok(())
        }
        StrategyPref::Auto | StrategyPref::Overlay => {
            ...
            match strategy::decide_after_overlay_attempt(require_overlay, mount_errno)? {
                ...
                Isolation::CopyIn { fell_back } => {
                    ...
                    let stats = populate_with_status(args, status, reason.as_deref())?;
                    status.done("copy-in", reason.as_deref(), Some(stats));
                    Ok(())
                }
            }
        }
...
) -> Result<copy_in::CopyStats, InitError> {
    status.copy(copy_in::CopyStats::default(), fell_back);
    let stats = copy_in::populate(&args.lower, &args.workspace, args.file_copy, &mut |stats| {
        status.copy(stats, fell_back);
    })?;
