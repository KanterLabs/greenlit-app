//! Static extraction: every `secrets.*`/`vars.*` reference, whether any
//! dynamic `vars[...]` lookup exists, every `uses:` reference, and every
...
//! ## Known limitation
//!
//! If an expression's interior text fails to parse (a malformed
//! `${{ ... }}` body), this module silently contributes nothing for that
//! occurrence rather than surfacing the parse error itself — extraction is
//! a best-effort pre-pass, not validation; the real, precise
//! [`greenlit_expr::ParseError`] surfaces later when `greenlit-engine`
//! actually plans the expression.

use crate::expression::find_closing_delimiter;
...
pub struct StaticExtraction {
    ...
    /// Whether a dynamic `vars[<non-literal>]` lookup appears anywhere.
    /// Per `greenlit-v0-spec.md` "Secrets/vars": "Dynamic `vars[...]`
    /// requires a complete locally supplied map in this phase" since the
    /// exact name referenced can't be known statically.
...
/// `vars[...]` lookup, `uses:` reference, and `runs-on` value.
#[must_use]
pub fn extract_static(workflow: &Workflow) -> StaticExtraction {
...
fn scan_job(job: &Job, out: &mut StaticExtraction) {
...
fn scan_strategy(strategy: &Strategy, out: &mut StaticExtraction) {
...
fn scan_matrix(matrix: &Matrix, out: &mut StaticExtraction) {
...
fn scan_container(c: &ContainerSpec, out: &mut StaticExtraction) {
...
fn scan_step(step: &Step, out: &mut StaticExtraction) {
...
fn scan_scalar_or_expr(v: &Spanned<ScalarOrExpr>, out: &mut StaticExtraction) {
    if let ScalarOrExpr::Expression(text) = &v.value {
        scan_wrapped_text(text, &v.span, out);
    }
}

fn scan_yaml_value(v: &Spanned<YamlValue>, out: &mut StaticExtraction) {
    match &v.value {
        YamlValue::Scalar(s) => {
...
fn scan_if_condition(cond: &Spanned<String>, out: &mut StaticExtraction) {
    if cond.value.contains("${{") {
        scan_wrapped_text(&cond.value, &cond.span, out);
    } else {
        scan_expression_body(&cond.value, &cond.span, out);
    }
}

/// Find every `${{ ... }}` occurrence in `text` and scan its interior.
fn scan_wrapped_text(text: &str, span: &Span, out: &mut StaticExtraction) {
    let mut rest = text;
    while let Some(start) = rest.find("${{") {
        let after_open = &rest[start + 3..];
        let Some(end) = find_closing_delimiter(after_open) else {
            break;
        };
        scan_expression_body(after_open[..end].trim(), span, out);
...
/// Parses one `${{ ... }}` body (or one bare `if:` expression) with the
/// real `greenlit-expr` grammar and walks it for `secrets.*`/`vars.*`
/// references and dynamic `vars[...]` lookups. A parse failure here is not
/// reported (see this module's "Known limitation" doc section) — it
/// contributes nothing to the extraction rather than erroring.
fn scan_expression_body(body: &str, span: &Span, out: &mut StaticExtraction) {
    if let Ok(expr) = greenlit_expr::parse(body) {
...
fn walk_expr(expr: &Expr, span: &Span, out: &mut StaticExtraction) {
    match expr {
        ...
        Expr::Wildcard { target } => walk_expr(target, span, out),
        Expr::Binary { lhs, rhs, .. } => {
