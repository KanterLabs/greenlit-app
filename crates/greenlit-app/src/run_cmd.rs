...
struct RunMetricsFinalizer {
    invocation: Invocation,
    masker: greenlit_engine::execution::Masker,
    outcome: RefCell<Option<Option<String>>>,
}

impl RunMetricsFinalizer {
    fn new(invocation: &Invocation, masker: greenlit_engine::execution::Masker) -> Self {
        Self {
    ...
    fn mask_error(&self, error: anyhow::Error) -> anyhow::Error {
        anyhow::anyhow!(self.masker.apply(&error.to_string()))
    }
    ...
    fn finalize(&self) -> anyhow::Result<()> {
        ...
        if let Err(error) = self.masker.ensure_healthy() {
            ...
            *self.outcome.borrow_mut() = Some(Some(message.clone()));
            return Err(anyhow::anyhow!("{message}"));
        }

        let mut record = self.invocation.clone().finish();
        mask_metrics_record(&mut record, &self.masker);
        let mut failures = Vec::new();
        if let Err(error) = render::diagnostics::render_timings(&record, &mut io::stderr()) {
        ...
        match encoded {
            Ok(encoded) if self.masker.apply(&encoded) == encoded => {
                if let Err(error) =
                    MetricsStore::open_default().and_then(|store| store.append(&record))
                {
                    let diagnostic = errors::metrics_error(&error);
                    failures.push(format!(
                        "could not append the sanitized local run metric: {diagnostic}"
                    ));
                }
            }
            Ok(_) => failures.push(
                "could not append the sanitized local run metric: the record still contained a credential-bearing value\n  fix: preserve the run directory and file a Greenlit defect"
                    .to_string(),
            ),
            Err(error) => failures.push(format!(
                "could not validate the sanitized local run metric: {error}\n  fix: update Greenlit, then retry"
            )),
        }
        let failure = (!failures.is_empty()).then(|| failures.join("\n"));
        *self.outcome.borrow_mut() = Some(failure.clone());
        failure.map_or(Ok(()), |message| Err(anyhow::anyhow!("{message}")))
    }
}

fn mask_metrics_record(
    record: &mut greenlit_metrics::InvocationRecord,
    masker: &greenlit_engine::execution::Masker,
) {
    record.command = masker.apply(&record.command);
    for stage in &mut record.stages {
        stage.name = masker.apply(&stage.name);
    }
    for step in &mut record.steps {
        step.job = masker.apply(&step.job);
        step.step = masker.apply(&step.step);
    }
    for counter in &mut record.hit_miss {
        counter.name = masker.apply(&counter.name);
    }
}
...
pub(crate) fn run(args: RunArgs) -> anyhow::Result<ExitCode> {
    let mut explicit_sensitive_values = crate::run_quarantine::explicit_sensitive_values(&args);
    explicit_sensitive_values.extend(
        args.inputs
            .iter()
            .chain(&args.vars)
            .map(|(_, value)| value.clone()),
    );
    let preflight_masker = greenlit_engine::execution::Masker::new();
    for value in &explicit_sensitive_values {
        preflight_masker
            .add(value)
            .map_err(|error| anyhow::anyhow!("{error}"))?;
    }
    let explicit_preflight = (|| {
        crate::run_quarantine::reject_explicit_dispatch_inputs(&args.inputs, args.allow_degraded)?;
        crate::run_quarantine::reject_explicit_variables(&args.vars, args.allow_degraded)?;
        if !args.secrets.is_empty() {
            anyhow::bail!(
                "run is blocked by uncertified capability 'secret.context' at command-line.--secret: explicit secrets have not completed credential-bearing preflight; this finding cannot be forced and was blocked before daemon, credential, network, action, or container work\n  fix: omit --secret until Phase 16 certifies secret preflight"
            );
        }
        Ok(())
    })();
    explicit_preflight.map_err(|error: anyhow::Error| {
        anyhow::anyhow!(preflight_masker.apply(&error.to_string()))
    })?;

    let invocation = Invocation::start("run");
    let metrics = RunMetricsFinalizer::new(&invocation, preflight_masker.clone());
    let outcome = invocation
        .with_timing_subscriber(|| execute(&args, &invocation, &metrics, &preflight_masker));
    // Terminal publication finalizes metrics before `result.json`. Only runs
    // which never reached a publication attempt need the outer finalizer.
    // Avoid replaying a cached failure here: a publication attempt already
    // incorporated that exact diagnostic into its command error.
    let metrics_outcome = if metrics.is_finalized() {
        Ok(())
    } else {
        metrics.finalize()
    };
    let result = match (outcome, metrics_outcome) {
        (Ok(code), Ok(())) => Ok(code),
        (Err(error), Ok(())) => Err(error),
        (Ok(_), Err(error)) => Err(error),
        (Err(primary), Err(metrics_error)) => Err(anyhow::anyhow!(
            "{primary}\nadditionally, metrics finalization failed: {metrics_error}"
        )),
    };
    result.map_err(|error| metrics.mask_error(error))
}
...
fn execute(
    ...
    let evidence = invocation.time_stage("source-freeze", || {
        crate::run_evidence::RunEvidence::capture(repo_root.as_path(), masker.clone())
    })?;
    let recorder = match crate::run_events::RunEventRecorder::create(
        &evidence.directory,
        &evidence.run_id,
        args.format,
        args.log_mode,
        ...
        Err(error) => {
            ...
            return evidence.finalize_command(Err(error), || {
                Err(anyhow::anyhow!(
                    "terminal recorder was unavailable after run evidence capture"
                ))
            });
        }
    };
    let command_outcome = (|| -> anyhow::Result<ExitCode> {
        ...
        let prepared = invocation.time_stage("plan", || {
            crate::run_quarantine::prepare(workflow, authored_extraction, &frozen_repo_root, args)
        })?;
        ...
        if let Err(error) = crate::run_quarantine::reject_blocked(&assessment) {
            ...
            let publication = publish_terminal_result(
                &recorder,
                &evidence,
            ...
            return Err(combine_publication_error(error, publication));
        }
        crate::run_quarantine::render_degraded(&assessment, &metrics.masker)?;
        ...
        let (actions_config, pinned_resolver) = invocation
            .time_stage("action-resolve", || {
                build_action_runtime_config(args.offline)
            })
            .map_err(|message| anyhow::anyhow!(message))?;
        ...
        let config = RunConfig {
            ...
            // Every credential this run resolves shares one in-memory masking
            // authority with runtime, events, evidence, and metrics.
            masker: masker.clone(),
            ...
            store: None,
            allow_external_network: !args.hermetic,
        ...
        let report = match execution_result {
            ...
            Err(error @ greenlit_runtime::ExecError::CapabilityQuarantined { .. }) => {
                let support = evidence.support_report();
                let primary = anyhow::anyhow!("{error}");
                let publication = publish_terminal_result(
                    &recorder,
                    &evidence,
                    ExecutionConclusion::Blocked,
                ...
                return Err(combine_publication_error(primary, publication));
            }
            Err(error) => {
                let support = evidence.support_report();
                let primary = anyhow::anyhow!("{error}");
    ...
    evidence.finalize_command(command_outcome, || {
        publish_terminal_result(
            &recorder,
            &evidence,
            ExecutionConclusion::PreparationFailed,
            evidence.support_report(),
            clean,
            args.hermetic,
            metrics,
        )
...
fn combine_publication_error(
    ...
    match publication {
        Ok(()) => primary,
        Err(publication) => {
            anyhow::anyhow!("{primary}\nadditionally, terminal publication failed: {publication}")
        }
    }
}
...
fn publish_terminal_result(
    ...
    if let Err(error) = metrics.finalize() {
        evidence.abandon_result_publication();
        return Err(error);
    }
    ...
    let terminal = match recorder.prepare_finish(
    ...
    if let Err(error) = recorder.persist_finish(&terminal) {
        evidence.abandon_result_publication();
        return match evidence.discard_uncommitted_tree() {
            Ok(()) => Err(error),
            Err(cleanup) => Err(anyhow::anyhow!(
                "{error}\nadditionally, could not discard the uncommitted terminal tree: {cleanup}"
            )),
        };
    }
    hold_terminal_publication("after-terminal-sync");
    if let Err(error) = evidence.publish_prepared_result(prepared) {
        evidence.abandon_result_publication();
        return Err(error);
    }
    hold_terminal_publication("after-catalog-complete");
    recorder.render_finish(terminal)?;
    Ok(())
}

fn hold_terminal_publication(_point: &str) {
    #[cfg(litci_test_boundaries)]
    while std::env::var_os("LITCI_TEST_TERMINAL_PUBLICATION_HOLD").as_deref()
...
/// Builds the Phase 12 credential-free action configuration.
///
/// Reachable actions are blocked before this function is called. Keeping the
/// existing unauthenticated resolver/fetcher configuration available for
/// action-free runs preserves the runtime shape while making credential
/// retrieval and injection unrepresentable until Phase 16 certifies them.
///
/// # Errors
...
> {
    use greenlit_actions::resolve::{
        GitLsRemoteResolver, PersistentRefResolver, PinnedRefResolver, RefResolver,
    };
    ...
    let inner_resolver: Arc<dyn RefResolver> = Arc::new(GitLsRemoteResolver::new());
