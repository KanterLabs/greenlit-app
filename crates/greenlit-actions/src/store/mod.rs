...
mod tests {
    ...
    struct WritesMarkerFile {
        calls: AtomicUsize,
    }

    #[async_trait::async_trait]
    impl ActionFetcher for WritesMarkerFile {
        async fn fetch(
            &self,
            _owner: &str,
            _repo: &str,
    ...
    #[async_trait::async_trait]
    impl ActionFetcher for AlwaysFails {
        async fn fetch(
            &self,
            owner: &str,
            repo: &str,
    ...
    #[test]
    fn default_path_under_matches_documented_layout() {
        let home = Path::new("/home/user");
        assert_eq!(
            ActionStore::default_path_under(home),
            Path::new("/home/user/.litci/actions")
        );
    }

    #[tokio::test]
    async fn miss_then_hit_calls_the_fetcher_exactly_once() {
        let root = tempfile::tempdir().unwrap();
        let store = ActionStore::at(root.path());
        let fetcher = WritesMarkerFile {
            calls: AtomicUsize::new(0),
        };
        let sha = test_sha();

        let (dir1, outcome1) = store
            .ensure_fetched("owner", "repo", &sha, &fetcher)
            .await
            .unwrap();
        assert_eq!(outcome1, FetchOutcome::Miss);
        assert_eq!(
            std::fs::read_to_string(dir1.join("action.yml")).unwrap(),
            "name: ok"
        );

        let (dir2, outcome2) = store
            .ensure_fetched("owner", "repo", &sha, &fetcher)
            .await
            .unwrap();
        ...
        // The store's own presence check, not the fetcher, must answer the
        // second call — this is the "zero network fetches for pinned SHAs
        // on re-run" invariant (`TESTING.md`), asserted at this crate's
        // fetch boundary via an interaction count on the injected fetcher.
        assert_eq!(fetcher.calls.load(Ordering::SeqCst), 1);
        assert_eq!(store.counts(), StoreCounts { hits: 1, misses: 1 });
    }

    #[tokio::test]
    async fn has_reflects_presence_without_fetching() {
        let root = tempfile::tempdir().unwrap();
        let store = ActionStore::at(root.path());
        let sha = test_sha();
        assert!(!store.has("owner", "repo", &sha).unwrap());
        store
            .ensure_fetched(
                "owner",
                "repo",
    ...
    #[tokio::test]
    async fn a_failed_fetch_leaves_no_directory_that_looks_like_a_hit() {
        let root = tempfile::tempdir().unwrap();
        let store = ActionStore::at(root.path());
        let sha = test_sha();

        let error = store
            .ensure_fetched("owner", "repo", &sha, &AlwaysFails)
            .await
            .unwrap_err();
        assert!(matches!(error, StoreError::Fetch(_)));
        assert!(!store.has("owner", "repo", &sha).unwrap());

        // No orphaned temp sibling should remain treated as, or later
        // confused for, a completed SHA directory: the `repo` directory
        // must not contain anything named exactly like the SHA.
        let repo_dir = root.path().join("owner").join("repo");
        if repo_dir.is_dir() {
            for entry in std::fs::read_dir(&repo_dir).unwrap() {
                let name = entry.unwrap().file_name();
                assert_ne!(name.to_str().unwrap(), sha.as_str());
            }
        }
    }

    #[tokio::test]
    async fn rejects_path_unsafe_owner() {
        let root = tempfile::tempdir().unwrap();
        let store = ActionStore::at(root.path());
        let sha = test_sha();
        let error = store.action_dir("../escape", "repo", &sha).unwrap_err();
        assert!(matches!(error, StoreError::InvalidComponent { .. }));
    }

    #[tokio::test]
    async fn different_shas_do_not_collide() {
        let root = tempfile::tempdir().unwrap();
        let store = ActionStore::at(root.path());
        ...
        let (dir_a, _) = store
            .ensure_fetched("owner", "repo", &sha_a, &fetcher)
            .await
            .unwrap();
        let (dir_b, _) = store
            .ensure_fetched("owner", "repo", &sha_b, &fetcher)
            .await
            .unwrap();
